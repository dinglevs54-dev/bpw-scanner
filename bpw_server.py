...
# File to store logs permanently
LOGS_FILE = "logs.json"

# In‑memory cache (loaded from file)
logs_cache = {}
sessions = {}  # For PIN generation status
...
def list_logs():
    if not session.get('logged_in'):
        return jsonify({"error": "Unauthorized"}), 401
    # Return a summary of all stored logs (PIN, timestamp, hostname, username)
    summary = []
    for pin, entry in logs_cache.items():
        if entry.get('status') == 'completed':
            logs = entry.get('logs', {})
...
# ============================================================================
# DASHBOARD HTML (updated with log list)
# ============================================================================
...
<div class="sidebar" id="sidebar">
    ...
    <div class="nav-item" onclick="showPage('logs')">
        <span class="icon"><i class="fas fa-history"></i></span> All Logs
        <span class="badge-nav" id="log-count">0</span>
    </div>
...
<div class="main-content">
    ...
    <div id="page-logs" class="page">
        <div class="glass-panel">
            <div class="panel-header">
                <div class="panel-title"><span class="emoji">📋</span> All Stored Logs</div>
                <button class="btn-secondary" onclick="refreshLogList()" style="padding: 6px 16px; font-size:12px;">🔄 Refresh</button>
            </div>
            <div id="log-list-container">
                <div class="empty-state">
                    <i class="fas fa-database"></i>
                    <h4>No logs stored yet</h4>
                    <p>Run a scan and upload results to see them here.</p>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    async function fetchLogs() {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                refreshLogList(); // also refresh the log list
            } else {
    ...
    async function refreshLogList() {
        try {
            const res = await fetch('/api/list-logs');
            const data = await res.json();
            storedLogs = data;
            renderLogList(data);
    ...
    function renderLogList(logs) {
        ...
        logs.forEach(log => {
            html += `
                <tr>
                    <td><span class="pin-code">${log.pin}</span></td>
                    <td>${new Date(log.timestamp).toLocaleString()}</td>
                    <td>${log.hostname}</td>
                    <td>${log.username}</td>
                    <td>${log.findings}</td>
                    <td><button class="view-btn" onclick="fetchLogsByPin('${log.pin}')">View</button></td>
                </tr>
            `;
        });
        html += `</tbody></table>`;
    ...
    async function fetchLogsByPin(pin) {
    ...
    function populateAllSections(logs) {
        // General info is not used here as we have a dedicated log list; skip.
